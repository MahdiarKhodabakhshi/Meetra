...
export default function ProblemStatement({ sectionRef, heroReady }: Props) {
  ...
  const eased = useTransform(scrollYProgress, [0.1, 0.6], [0, 1], {
    clamp: true,
  });
  ...
  // "ra" fades out
  const raOpacity = useTransform(eased, [0.2, 0.6], [1, 0], {
    clamp: true,
  });

  // Text reveals
  const textOpacity = useTransform(eased, [0.45, 0.85], [0, 1], {
    clamp: true,
  });
  const textY = useTransform(eased, [0.45, 0.85], [16, 0], {
    clamp: true,
  });

  const line2Opacity = useTransform(eased, [0.55, 0.9], [0, 1], {
    clamp: true,
  });
  const line2Y = useTransform(eased, [0.55, 0.9], [24, 0], {
    clamp: true,
  });
  ...
  return (
    <div ref={sectionRef} className="relative bg-[#0A0F1C] h-screen mb-16">
      <div className="h-full flex items-center justify-center overflow-hidden">
        <div className="w-full px-6 max-w-[1500px] text-center relative z-10">
          <h2 className="font-[family-name:var(--font-playfair)] text-[clamp(1.8rem,4.2vw,3.5rem)] font-medium tracking-[-0.025em] leading-[1.3]">
            <span className="block text-white md:whitespace-nowrap">
              <motion.span
                ref={meetRef}
                className="inline-block relative mr-[0.18em]"
                style={{
                  x: meetTx,
                  y: meetTy,
                  scale: meetScale,
              ...
              >
                <span className="text-[88px] text-[#60A5FA]">Stop</span>
                <motion.span
              ...
              >
                leaving event networking to chance.
              </motion.span>
            </span>

            <motion.span
  className="mt-6 block text-[clamp(1.6rem,3.2vw,3rem)] leading-[1.15] text-white/50 md:whitespace-nowrap"
  style={{ opacity: line2Opacity, y: line2Y }}
>
  Meet the right people with a clearer plan.
</motion.span>
          </h2>
        </div>
      </div>
