...
export default function ProblemStatement() {
  ...
  // Text: blurry/dim → sharp/white when Meet arrives (~0.5 scroll)
  const blurVal = useTransform(scrollYProgress, [0.4, 0.55], [8, 0]);
  const textFilter = useMotionTemplate`blur(${blurVal}px)`;
  const textOpacity = useTransform(scrollYProgress, [0.4, 0.55], [0.25, 1]);
